...
- No `staff_audit` trigger: both inserts happen explicitly inside this function, so `ensure_staff()`'s own per-page-load refresh of `display_name`/`username`/`last_signed_in_at` never touches the audit table.
...
| `getAllStaff()`              | server read, `lib/staff.ts`          | none                                                                     | every `staff` row: name, username, role, active, last signed in, version | `staffSupabase()`, relies on the existing "active staff may read the staff list" select policy | a thrown error surfaces as the page's error notice                            |
...
12. [x] Revision, drop the confirm step (since reversed, see step 15): `ConfirmDialog` removed from this feature; `UserSheet`'s Save button, labelled `Continue` before this revision, called `updateStaffRole` directly. The sheet also gained an `Email` label above the address, matching `Role`'s, its title became `Edit <name>`, and the active field's helper text now says "Deactivate" rather than "Switched off".
...
15. [x] Revision, confirm step reinstated (2026-09-21): the sheet's button reads `Continue` again and hands the proposed role and active flag to `UsersPanel`, which opens `ConfirmDialog` (`Save` / `Go back`, destructive styling when the change deactivates someone) with the changed fields spelled out, including the "will become Admin at the same time" note on an owner transfer, closing that follow-up. The write, the stale handling and the toasts are unchanged; refusals now show inside the dialog instead of a toast. Satisfies the revised **AC-4**.
...
- [x] The "`<person>` will become admin at the same time" warning on an owner transfer is back in the confirm dialog (build plan step 15).
