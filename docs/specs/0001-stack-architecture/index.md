...
| Hosting                 | Self hosted Docker container on a VPS, Next.js `output: "standalone"`                             | Engineer's choice. Provider not yet committed, so the build targets any Docker capable host.                                                                                                          |
...
- [ ] Pick the hosting provider and deploy panel before the scaffold ships. The build targets any Docker capable host, but TLS, the domain and the deploy trigger need a real answer.
