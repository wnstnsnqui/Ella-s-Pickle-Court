...
- **AC-3**: When the day shown is venue today, a compact strip above the grid lists every court in sort order with one of: "Free now", "Free from 3pm" (the label of the first Available row ending after now, formatted with `formatSlotLabel`; "Free now" when that row has already started), or "Nothing free today". The strip is absent on any other day. It is a list with text, readable by a screen reader, not colour alone.
- **AC-4**: On venue today, rows whose slot has ended are rendered dimmed (a row level treatment, keeping every cell's label and icon, no lock icon), a "Now" marker sits before the first row that has not ended, and on first paint the marker element is scrolled into view at the top (instantly, never animated, and never again after a refetch). On other days nothing is dimmed and no marker shows. The time used is the server's `now`, ticked forward on the client once a minute, so a device with a wrong clock cannot move the marker. The day navigation uses the same `now` to decide which day is today.
- **AC-5**: The board joins the private `schedule` broadcast topic through the anonymous browser client (no Clerk token, ever), re reads the whole day through `GET /api/schedule` on every broadcast rather than patching from the payload, coalesces broadcasts arriving within 300 ms into one read, never starts two reads less than 2 seconds apart in one tab (a trigger inside that gap becomes one read at its end), and only lets the newest read land. A booking made on the staff board shows as Booked on an already open public board within two seconds with no reload, with the spec 0003 changed cell highlight and the live indicator reading live.
...
- **AC-9**: Every trigger (a broadcast, a poll tick, the tab becoming visible, the channel recovering, the day rolling over) passes through one gate: it marks a read as wanted, and a read starts only when the 300 ms window has elapsed, the 2 second floor has passed, and no wait is active. On a `429` from `GET /api/schedule`, the board shows the not live reading with the data age, sets a wait of `Retry-After` seconds (a further `429` sets a fresh wait from its own header, never compounding), keeps the last good grid, and when the wait ends runs one read if any trigger fired during it. On any other failed read the board keeps the last good grid and shows the not live reading; a failed first render shows the spec 0003 error state with Retry.
- **AC-10**: A board opened with no `?date=` follows the venue's day. The client minute clock compares `calendarDateInZone(now, grid.timezone)` with `grid.date` on every tick, and the moment they differ it triggers one read through the AC-9 gate; when any read returns a different `grid.date`, the board re keys to the new day, the strip and marker follow, and the day navigation reflects the new today. A board opened with a `?date=` stays on that date.
- **AC-11**: Metadata: `/` carries the venue title (`Ella's Picklecourt · Court schedule`), the tagline as description, the existing generated social card, and is indexable; `/?date=…` carries the day in its title (`Court schedule for Sat 20 Sep · Ella's Picklecourt`, venue local) and a canonical link to `/`. The page embeds one JSON-LD `SportsActivityLocation` block with the venue name, URL and an `openingHoursSpecification` derived from `schedule.hours` (the weekday and weekend open and close times) at render time. `generateMetadata` and the page share one `getSchedule()` call per request through React `cache()`. Nothing personal and no reservation data goes in the block.
...
- **AC-13**: The channel logic lives once: a base hook `useScheduleChannel<T extends Schedule>` owns the channel, the read gate (coalescing, floor, wait), the generation guard, the status, the focus refetch and the optional poll; it takes the Supabase client, the initial schedule, a `transport` and an optional `prepare` step run before subscribing and on demand. The staff hook supplies `staffBrowserSupabase()`, a `prepare` that applies the Clerk token, and a transport wrapping `refreshStaffSchedule`; the public hook supplies `browserSupabase()`, a `prepare` that calls `realtime.setAuth()` with no argument, a transport wrapping `fetch("/api/schedule")` that reads `Retry-After`, and `pollWhileDownMs: 60_000`. The staff board's behaviour is unchanged and its existing tests pass.
...
- **The `now` stamp and the hours travel on the schedule.** `Schedule` gains `now: string` (a UTC instant) and `hours: { weekdayOpen, weekdayClose, weekendOpen, weekendClose }` (venue local `HH:mm`), set by both `getSchedule()` and `getStaffSchedule()` from the settings they already load. The client starts its minute clock from that stamp and moves it forward by elapsed time. Runner up: a separate header or field on the route only, which leaves the first server render without a `now` and reopens the hydration mismatch the question was about.
...
- **The limiter is a sliding window of timestamps per key.** `lib/rate-limit.ts` exports a pure `SlidingWindow` (limit, window, a clock injected for tests) with `hit(key, now)` returning `{ allowed, retryAfterSeconds }`, pruning each key on hit, a sweep every 60 s that drops keys with no timestamp left in the window, and a cap of 10,000 keys where an insert past the cap evicts the oldest inserted key (Map order), so memory stays bounded. Runner up: a fixed window counter (fewer timestamps kept, but it lets 120 through at a window boundary).
- **The limiter runs before Clerk, on the Node runtime.** `proxy()` checks the limit first for the two public paths and returns `429` without invoking `clerkMiddleware`. `proxy.ts` runs on the Node runtime (the Next.js 16 default, to be confirmed against `node_modules/next/dist/docs/` at build time), so the module level map is one instance per container process. Runner up: inside the Clerk handler, which spends Clerk's work on a request about to be refused.
- **The strip's "Free now" means the current slot is Available.** `nextFreeTime(grid, courtId, now)` changes to return the first Available `GridRow` ending after `now` (or `null`) instead of its start instant, so the strip has the row's `label`; when `row.startsAt <= now` the strip says "Free now", otherwise "Free from" plus `formatSlotLabel(row.label)`; `null` reads "Nothing free today". Its one existing use and test move with it. Runner up: a separate "closed today" wording when every slot is Unavailable, which needs a second derivation for a rare case; the grid itself shows it.
- **Scroll to now is a one shot effect keyed on the grid date.** `ScheduleGrid` renders the marker as its own element and exposes it through a `markerRef`; the board calls `scrollIntoView({ block: "start" })` on it after mount when `grid.date` is venue today, with no smooth behaviour, and never again on refetch.
- **The day navigation takes the same clock.** `DayNav` gains an optional `now?: string` passed into `todayInZone`; both boards pass `schedule.now`, so "today" and the horizon edge never come from the device clock.
- **One read gate, four timers.** The base hook keeps `wanted`, `lastReadAt`, `waitUntil` and the 300 ms coalescing timer; every trigger sets `wanted` and asks the gate, and the gate starts a read only when the window has elapsed, `Date.now() - lastReadAt >= 2000`, and `waitUntil` has passed, scheduling itself for the earliest moment that becomes true. Runner up: separate timers per trigger, which is how a poll tick lands inside a 429 wait.
- **The transport contract.** `type ScheduleTransport<T> = (date?: string) => Promise<{ ok: true; data: T } | { ok: false; message: string; retryAfterMs?: number }>`. The staff transport maps `ActionResult` onto it; the public transport maps the JSON body and, on `429`, `Retry-After` (seconds, default 5 when missing) onto `retryAfterMs`. `ActionError` itself does not grow a new kind. Runner up: adding `rate_limited` to `ActionError`, which widens a union every Server Action switch must handle for a case no action can produce.
- **The public read is memoised per request.** `getSchedule` is exported wrapped in React `cache()` so `generateMetadata` and the page body share one database round trip. Runner up: passing the result from the page into the metadata, which Next.js does not allow.
- **The anonymous prepare step is `realtime.setAuth()` with no argument.** That is what lets the policy on `realtime.messages` be evaluated for `anon` at join time, per `lib/supabase/AGENTS.md`, and the scaffold's smoke channel already proved anon on a private topic. Runner up: skipping the call, which the gotcha in that file exists to warn against. Runner up: a sticky "Jump to now" button, which is a second control for something the page can just do.
...
The only type changes are `Schedule` gaining `now: string` and `hours` (the four venue local open and close times), and `nextFreeTime()` returning a `GridRow`. `StaffSchedule` inherits the two fields.
...
| render `/`             | the next free strip lines                        | derived: `nextFreeTime(grid, court.id, now)` returns the row; "Free now" when `row.startsAt <= now`, else `formatSlotLabel(row.label)`; shown only when `grid.date === calendarDateInZone(now, grid.timezone)` |
...
| render `/`             | the JSON-LD opening hours                        | `schedule.hours`, filled by `getSchedule()` from `venue_settings`                                                                                       |
| render `/`             | one read for metadata and body                   | `getSchedule` wrapped in React `cache()`                                                                                                             |
| render `/`             | which day the navigation calls today             | `todayInZone(grid.timezone, now)` with `now` from the schedule, passed to `DayNav`                                                                     |
| render `/`             | the scroll target                                | the marker element exposed by `ScheduleGrid` through `markerRef`                                                                                      |
...
| listener               | when to re read                                  | the `reservation_changed` broadcast (spec 0002 trigger), tab visibility, the 60 s poll while not live, the venue day rolling over, all through one gate with a 300 ms window, a 2 s floor and the `Retry-After` wait |
| listener               | that the venue day rolled over                   | derived on each minute tick: `calendarDateInZone(now, grid.timezone) !== grid.date`, on an undated board only                                          |
| listener               | the `429` wait                                   | `Retry-After` from the response, seconds, default 5 when absent                                                                                       |
...
7. The limiter is per address, in memory, and never keys on anything a client can set other than the forwarded address the host's reverse proxy writes. It runs before Clerk and before any database work, on the Node runtime, one map per process.
10. One tab never starts two reads less than 2 seconds apart, and never reads during a `429` wait. Every trigger goes through the gate; none calls the transport directly.
11. The day navigation, the strip, the dimming and the marker all read the same `now`.
...
- Bursts and staleness: six broadcasts within 100 ms produce one `GET /api/schedule`; a broadcast 500 ms after a read produces one more read 2 s after the first; a slow first response arriving after a fast second one is dropped, verifies **AC-5**.
...
- Backoff: a `429` with `Retry-After: 7` puts the board in not live, three broadcasts and one poll tick during those seven seconds produce exactly one read after them, and a second `429` with `Retry-After: 3` sets a three second wait, not ten, verifies **AC-9**.
- Midnight: with a stable channel and no broadcasts, a board opened without `?date=` triggers one read on the first minute tick after the venue day changes, re keys to the new `grid.date`, the strip, marker and day navigation follow, and a board opened with `?date=` for the old day stays on it, verifies **AC-10**.
- Metadata: `/` has the venue title, description, the social card, a canonical of `/` and the JSON-LD block with the settings' hours, and one request issues one settings read; `/?date=2026-09-20` has the day in its title and a canonical of `/`, verifies **AC-11**.
...
- Shared hook: every existing `use-staff-schedule` test passes unchanged after the move onto the base hook; the base hook's own tests cover the gate with a fake transport and a fake clock, verifies **AC-13**.
...
1. [x] The thin thread: `getSchedule()` gains `now` and `hours` on `Schedule`, is wrapped in React `cache()`, and `resolveDate` gains `allowPast` (public `false`, staff `true`); `DayNav` gains `now`; `app/page.tsx` becomes the board, `force-dynamic`, rendering `ScheduleGrid` read only with the four public legend views, `DayNav` in the toolbar, the "That day could not be shown" notice for a past, far or malformed date, and the spec 0003 empty and error states; `app/loading.tsx` with the skeleton; the holding copy goes. Prove it: a booking made on `/staff` shows Booked on `/` after a reload, in a browser with no session, satisfies **AC-1**, **AC-2**, **AC-12**.
2. [x] The endpoint: `app/api/schedule/route.ts`, `GET` only, `date` parsed with `scheduleDateSchema`, `getSchedule()` behind it, the `ActionResult` shape with `422` and `500`, `Cache-Control: no-store`, and a route test, satisfies **AC-2**, **AC-5**.
3. [x] The shared hook: extract `components/schedule/use-schedule-channel.ts` (generic over the schedule type; the channel on a given client, the `prepare` step, the `ScheduleTransport` contract, the read gate with its 300 ms window, 2 s floor and wait, the generation guard, the status, the visibility refetch, the optional poll, the listener set) from `use-staff-schedule.ts`, with its own tests on a fake transport and clock; move the staff hook onto it with only the Clerk token `prepare` and the Server Action transport left in it; every existing staff test passes, satisfies **AC-13**.
4. [x] Live: `components/board/use-public-schedule.ts` on the base hook with the anonymous client, a `prepare` of `realtime.setAuth()` with no argument, the JSON transport hitting `/api/schedule` (dated or undated to match how the page was opened) and reading `Retry-After`, and `components/board/public-board.tsx` rendering the grid, the changed cell highlight and the live indicator. Prove it: the booking from task 1 appears in a second, signed out browser with no reload, satisfies **AC-5**.
5. [x] The privacy proof: the serialisation test over `GET /api/schedule`, the four column select assertion, and the rendered HTML check for a day with a named booking, satisfies **AC-7**.
6. [x] Not live, backoff and midnight: the 60 s poll while not `SUBSCRIBED`, the single read on recovery, the `429` wait in the gate, the last good grid kept on any failure, the minute tick's venue day check on an undated board, and the re key on a changed `grid.date`, satisfies **AC-6**, **AC-9**, **AC-10**.
7. [x] The limiter: `lib/rate-limit.ts` (`SlidingWindow`, injected clock, prune on hit, 60 s sweep, 10,000 key cap) with unit tests; `proxy.ts`, confirmed on the Node runtime, applies it to `GET /` and `GET /api/schedule` before Clerk, keyed on the forwarded address, skipping a request with no address, answering `429` with `Retry-After` and the plain text or JSON body; `proxy.test.ts` extended, satisfies **AC-8**.
8. [x] Now on the grid: `now?: string` and `markerRef` on `ScheduleGrid` (dimmed `data-past` rows, the marker with visible text before the first live row), the client minute clock started from `schedule.now` and reset by every re read, the one shot scroll to the marker on today, `nextFreeTime()` returning the row, and `components/board/next-free-strip.tsx` shown only on today; the design page gets the marker and a dimmed row in its gallery, satisfies **AC-3**, **AC-4**.
9. [x] Metadata: `generateMetadata` on the page (venue title on `/`, the day in the title when dated, description, canonical `/`, `robots` index) sharing the cached read, and the JSON-LD `SportsActivityLocation` block from `schedule.hours` and `NEXT_PUBLIC_SITE_URL`; the existing social card is kept as is, satisfies **AC-11**.
10. [x] `npm run check` clean, the two browser proof in `verify.md`, and the holding page's tests replaced by the board's, satisfies every AC above. _Built on 2026-09-14: `npm run check` green (292 tests); a real browser on `/?date=2026-09-16` saw a closure inserted through the Supabase CLI turn Court 1 at 3pm Unavailable with the highlight and back to Available on delete, no reload, one endpoint read per change; the limiter refused the 61st request from one forwarded address with `Retry-After` while another address and a headerless request passed._
...
- A shared address (a clubhouse wifi) shares one bucket. The 2 second floor caps any one tab at 30 reads a minute, so two busy tabs on one address fit and three do not during a very busy hour. If that ever shows up as real `429`s, the limit is one constant to raise.
...
- `Schedule` gains `now` and `hours`, and `nextFreeTime()` returns a row. Every consumer already treats `Schedule` as a whole object, so the fields are additive; the one `nextFreeTime()` call site moves with it.
- `DayNav` and `ScheduleGrid` each gain a small optional prop. The staff board passes `now` to `DayNav` in this feature so both boards agree on today.
