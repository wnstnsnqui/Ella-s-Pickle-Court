...
# Spec 0009: empty by default, so a build with no PostHog project still works.
...
# The secrets (BETTER_AUTH_SECRET, BETTER_AUTH_DATABASE_URL, SUPABASE_JWT_SECRET,
# BOOTSTRAP_OWNER_USERNAME) are passed at runtime, never baked into the image.
