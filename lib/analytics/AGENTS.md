...
- `onRequestError` in `instrumentation.ts` runs outside any request store, so it cannot use `currentSession()`; it rebuilds a `Headers` object from the raw request and calls `auth.api.getSession()` directly, inside a try/catch. The value is attribution only, never an authorization check, and an unreadable session never stops the capture.
